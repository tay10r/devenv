FROM ubuntu:25.10
RUN apt-get update
RUN apt-get install -y g++ cmake git python3 python3-pip python3-venv neovim curl zip unzip tar clangd clang-format pkg-config
...
RUN cd vcpkg && ./vcpkg install \
  gtest spdlog cxxopts opencv[dnn] protobuf nlohmann-json eigen3 glm libuv pybind11
